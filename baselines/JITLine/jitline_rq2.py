...
from sklearn.metrics import confusion_matrix, roc_auc_score, matthews_corrcoef, precision_recall_fscore_support, \
    classification_report, auc,recall_score
...
def eval_metrics(result_df):
    ...
    gmean=np.sqrt(
            recall_score(y_test, pred, pos_label=0, average="binary") *
    ...
    return f1, AUC,gmean, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt


def eval_result(proj_name, sampling_method='DE_SMOTE_min_df_3'):
    data_path = 'data/jitline/'
    RF_data_dir = 'data/jitline/' + style + '/'
    file_name=f"{RF_data_dir}{proj_name}_RF_{sampling_method}_prediction_result.csv"
        
    RF_result = pd.read_csv(file_name)
    ...
    f1, auc,gmean, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt = eval_metrics(
        RF_result)
    print(
        'F1: {:.4f}, AUC: {:.4f},gmean:{:.4f}, PCI@20%LOC: {:.4f}, Effort@20%Recall: {:.4f}, POpt: {:.4f}'.format(
            f1, auc,gmean, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt))
    ...
    ResultWriter().write_result(result_path=result_path, method_name="JITLine", presults=presults)
