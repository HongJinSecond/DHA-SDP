...
from baselines.CC2Vec.jit_utils import mini_batches
import torch
...
def extracted_cc2ftr(data, params):
    ...
    par_dir = "data/{}/".format(params.project_name)
    if not os.path.isdir(par_dir):
        os.makedirs(par_dir)


    filename = os.path.join(par_dir, params.name)
    with open(filename,"wb") as file:
        # pickle.dump(commit_ftrs, open(params.name, 'wb'))
        pickle.dump(commit_ftrs, file)
