...
from data_aware import main as train_lora


'''
This is a script to run the comparision experiment for the Data-Driven Aware strategy.
'''


columns = ["style", "accuracy", "precision", "recall", "f1", "gmean", "mcc", "auc_score"]
...
def compare_test(args, test_dataset_dict, base_model,cluster_manager,weight_dict:dict):
    ...
    for lora_name, test_cluster_datasets in test_dataset_dict.items():
        ...
        print(f"style: {lora_name}: {len(test_cluster_datasets)} samples.")
        ...
        if lora_name != -1:
            finalModel.load_lora(
                os.path.join(LORA_DIR,
                             f"{args.cluster_model}/{args.n_cluster}/{args.base_model}/{args.pretrained_model}/{lora_name}"),
                lora_name=str(lora_name))
            finalModel.change_loras(str(lora_name))

        for batch in test_dataloader:
            outlier_mask=cluster_manager.checkOutliers(batch[3].numpy(),lora_key=lora_name,strategy=args.strategy)
            ...
            with torch.no_grad():
                ...
                weight=weight_dict.get(lora_name)
...
def main(args):
    ...
    ensure_directory_exists(os.path.join(RESULTS_DIR, f"{args.cluster_model}"))

    test_dataset_split=cluster_manager.split_datasets(test_dataset)
    valid_dataset_split=cluster_manager.split_datasets(valid_dataset)
    set_seed(args)
    weight_dict=weight_calculator(args,valid_dataset_split,mymodel,cluster_manager)
    compare_test(args,test_dataset_split,mymodel,cluster_manager,weight_dict)
