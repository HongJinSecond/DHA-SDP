"""
n_cluster_comparison_cct5.py

Compare validation performance across different n_cluster values [2, 4, 6, 8]
for the CCT5 encoder (CodeChangeModel). This is a separate file from
n_cluster_comparison.py because CCT5 has a fundamentally different API:
  - Different model loading (build_or_load_gen_model vs build_model_tokenizer_config)
  - Different forward call signature (keyword args, returns logits not prob)
  - Different probability extraction (softmax on logits)
  - Different early stopping metric (maximize gmean vs minimize loss)

Supports three clustering modes: style (KMeans), project (Hierarchical), developer (Hierarchical).

Usage:
    python n_cluster_comparison_cct5.py --do_train
    (Edit CLUSTER_TYPE, N_CLUSTERS_LIST below to configure)
"""
...
from utils.util import (parse_jit_args, set_seed, ensure_directory_exists, get_peft_lora_config)
...
RESUME = True
PLOT_METRIC = "f1"  # metric to plot on y-axis (choose from METRIC_KEYS)
...
def evaluate_model(args, eval_dataset, mymodel):
    """Evaluate CCT5 model, return metrics dict with eval_gmean (for early stopping)."""
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            ...
            prob = torch.nn.functional.softmax(logits, dim=1).data.cpu().numpy()[:, 1]
    ...
    result["eval_gmean"] = result["gmean"]
    return result


def get_predictions(args, eval_dataset, mymodel):
    """Evaluate CCT5 model, return raw (pred_prob, true_label) for aggregation."""
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.batch_size, num_workers=4)
...
def train_one_lora(args, train_dataset, eval_dataset, mymodel, style_name):
    """
    Train one LoRA adapter for CCT5. After training, restore the best LoRA weights
    (by eval_gmean, maximized) in memory so the caller can evaluate directly.
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            ...
            # Eval + early stopping (maximize gmean, matching cct5test.py)
            if (step + 1) % args.save_steps == 0 and eval_dataset is not None:
                ...
                if results["eval_gmean"] > best_gmean:
                    ...
                    print(f"    Find new best gmean {best_gmean:.4f}, saved best LoRA for '{style_name}'")
                else:
                    patience += 1
...
def build_base_model_cct5(args, device):
    """Build CCT5 base model and load pretrained checkpoint."""
    config, model, tokenizer = build_or_load_gen_model(args, load_model=False)
    model.init_MF_classifier()
    checkpoint_path = os.path.join(args.output_dir, "checkpoints/concat/cct5.bin")
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"CCT5 checkpoint not found: {checkpoint_path}")
...
def run_one_n_cluster(args, device, tokenizer, n_cluster, cluster_type):
    ...
    print(f"\n{'='*60}")
    print(f"  cluster_type={cluster_type}, n_cluster={n_cluster}, encoder=cct5")
    print(f"{'='*60}")
    ...
    for cluster_name, train_lora_dataset in train_cluster_dict.items():
        ...
        mymodel, _, _ = build_base_model_cct5(args, device)
...
def plot_all_from_csv():
    ...
    for encoder in encoders:
        ...
        ax.set_xlabel("n_cluster", fontsize=13)
        ax.set_ylabel(metric, fontsize=13)
        ax.set_title(f"{metric} vs n_cluster  (encoder={encoder})", fontsize=14)
        ax.legend(loc="best", fontsize=10)
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plot_path = os.path.join(by_encoder_dir, f"{encoder}_{metric}.{PLOT_FORMAT}")
    ...
    for cluster_type in cluster_types:
        ...
        for encoder in encoders:
            ...
            ax.plot([r["n_cluster"] for r in rows],
                    [r[metric] for r in rows],
                    marker="o", linewidth=2, markersize=8, label=encoder)
...
def main(args):
    ...
    # Apply configuration
    args.pretrained_model = "cct5"
    args.base_model = "concat"
    args.do_train = True

    # Build tokenizer once (reused across all runs)
    # build_or_load_gen_model returns (config, model, tokenizer)
    _, _, tokenizer = build_or_load_gen_model(args, load_model=False)
    ...
    for cluster_type in CLUSTER_TYPES:
        print(f"\n{'#'*60}")
        print(f"#  Running cluster_type = {cluster_type}, encoder = cct5")
        print(f"{'#'*60}")

        # Open CSV for this cluster_type + encoder (overwrite previous partial results)
        csv_path = os.path.join(output_dir, f"{cluster_type}_cct5_comparison.csv")
        ...
        if results:
            print_comparison_table(results, cluster_type, "cct5")
