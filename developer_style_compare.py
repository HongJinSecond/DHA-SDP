...
from utils.util import parse_jit_args, set_seed, build_model_tokenizer_config,get_peft_lora_config,ensure_directory_exists
from utils.process_datasets import load_developer_datas
from models.ConcatModel import ConcatModel
from config import *
from developer_style_aware import main as train_lora
...
from utils.util import write_test_results_to_csv

'''
This is a script to run the comparision experiment for the Developer-Aware strategy.
'''

columns = ["developer", "accuracy", "precision", "recall", "f1", "gmean", "mcc", "auc_score"]
...
def compare_test(args, test_dataset_dict, base_model,cluster_manager:HierarchicalCluster,weight_dict):
    #load model
    base_model.eval()
    ...
    for developer_name, test_cluster_datasets in test_dataset_dict.items():
        ...
        print(f"developer: {developer_name}: {len(test_cluster_datasets)} samples.")
        ...
        if developer_name != "other":
            finalModel.load_lora(
                os.path.join(LORA_DIR, f"{args.cluster_model}/{args.n_cluster}/{args.base_model}/{args.pretrained_model}/{developer_name}"),
                lora_name=str(developer_name))
            finalModel.change_loras(str(developer_name))

        for batch in test_dataloader:
            if developer_name=="other":
                outlier_mask=None
            else:
                outlier_mask=cluster_manager.checkOutliers(batch[3].numpy(),lora_key=developer_name,strategy=args.strategy)
        ...
        write_test_results_to_csv(commit_ids=commit_hashs,
        ...
        for base_model in ["concat"]:
            ...
            # for pretrained in ["codebert"]:
                ...
                plot_comparison(
                    ...
                    metrics=["f1", "gmean","mcc","recall"],  # Metrics
